...
class SlotAttentionWMask(SlotAttention):
    ...
    def forward(self, inputs, slots):
        ...
        return slots, seg_mask, F.softmax(attn_logits, dim=-1)
...
class SAViDiffusion(SAVi):
    ...
    def encode(self, img, prev_slots=None):
        ...
        for idx in range(T):
            ...
            slots, masks, _ = self.slot_attention(encoder_out[:, idx], latents)
...
class ConsistentSAViDiffusion(SAVi):
    """SlotDiffusion model on videos."""
    ...
    def encode(self, img, prev_slots=None):
        ...
        for idx in range(T):
            ...
            # perform SA to get `slots` and `masks`
            slots, masks, attns = self.slot_attention(encoder_out[:, idx], latents)
            all_slots.append(slots)
            all_masks.append(masks)
    ...
    def _forward(self, img, prev_slots=None):
        ...
        # encode
        slots, masks, attn_maps = self.encode(img, prev_slots=prev_slots)
        # `slots` has shape: [B, T, self.num_slots, self.slot_size]
        # `masks` has shape: [B, T, self.num_slots, H, W]

        out_dict = {'masks': masks, 'slots': slots, 'attn_maps': attn_maps}
        if self.testing:
            return out_dict
        ...
        if self.use_consistency_loss:
            if self.pred_dict['const_type'] == 'repr':
                ...
                for t in range(out_dict['slots'].shape[1]-1):
                    ...
                    pairwise_sim = torch.bmm(z_curr.transpose(1, 2), z_next) # cosine similarity 
            ...
            loss_dict.update({'consistency_loss': cosine_loss})
        return loss_dict
